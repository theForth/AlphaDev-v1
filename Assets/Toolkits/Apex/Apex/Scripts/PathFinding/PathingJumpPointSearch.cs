...
namespace Apex.PathFinding
{
    ...
    public class PathingJumpPointSearch : PathingAStar
    {
        ...
        /// <param name="cutCorners">if set to <c>true</c> the path will cut corners. Cutting corners is cheaper, but less natural.</param>
        public PathingJumpPointSearch(int heapInitialSize, bool cutCorners)
            : base(heapInitialSize, cutCorners)
        {
        ...
        public PathingJumpPointSearch(int heapInitialSize, bool cutCorners, IMoveCost moveCostProvider, ISmoothPaths pathSmoother)
            : base(heapInitialSize, cutCorners, moveCostProvider, pathSmoother)
        {
            _neighbours = new DynamicArray<IPathNode>(8);
        }
        ...
        protected override void GetWalkableSuccessors(IPathNode current, DynamicArray<IPathNode> successorArray)
        {
            ...
            {
                current.GetWalkableNeighbours(_neighbours, _requesterAttributes, this.cutCorners, false);
            }
            else if (current is IPortalNode)
            {
                current.GetWalkableNeighbours(successorArray, _requesterAttributes, this.cutCorners, false);
                return;
            }
            else
            {
        ...
        private IPathNode Jump(IPathNode current, VectorXZ direction)
        {
            ...
            if (direction.x != 0 && direction.z != 0)
            {
                ...
                bool jumpOn = this.cutCorners ? ((n1 != null && n1.isWalkableFrom(current, _requesterAttributes)) || (n2 != null && n2.isWalkableFrom(current, _requesterAttributes))) : ((n1 != null && n1.isWalkableFrom(current, _requesterAttributes)) && (n2 != null && n2.isWalkableFrom(current, _requesterAttributes)));
                if (jumpOn)
                {

...
namespace Apex.Steering.Components
{
    ...
    {
        /// <summary>
        /// The priority with with this unit's path requests should be processed
        /// </summary>
        ...
        /// </summary>
        public bool usePathSmooting = true;
        ...
        /// </summary>
        public SlowingAlgortim slowingAlgoritm = SlowingAlgortim.Logarithmic;
        ...
        protected override Vector3 GetMovementVector(Vector3 currentVelocity)
        {
            ...
            if (_currentDestinationDistance < _cellRadius)
            {
        ...
        public override float GetSpeedAdjustmentFactor(Vector3 currentVelocity)
        {
            if (_onFinalApproach)
            {
                ...
                if (slowingAlgoritm == SlowingAlgortim.Linear)
                {
        ...
        private void RequestPath(Vector3 from, Vector3 to, RequestType type)
        {
            lock (_syncLock)
            {
                _pendingPathRequest = new BasicPathRequest()
                {
                    ...
                    usePathSmooting = this.usePathSmooting,
        ...
        private void ConsumeResult()
        {
            ...
            SetCurrentGrid(result.originalRequest.fromGrid);
        ...
        private void SetCurrentGrid(IGrid grid)
        {
            _currentGrid = grid;

            //Get the grid cell size. If we are moving outside a grid just use a default of 2.
            int gridStep = 2;
            if (_currentGrid != null)
            {
                gridStep = _currentGrid.cellSize;
            }

            _cellRadius = gridStep / 2.0f;
        }

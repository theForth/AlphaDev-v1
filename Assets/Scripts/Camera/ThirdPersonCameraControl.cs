...
public class ThirdPersonCameraControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            HOTween.To(this, 1f, "targetOffset", new Vector2(2f, 2f));
            // currentCameraState = CameraState.ThirdPerson;
        }
        if (Input.GetKeyDown(KeyCode.F))
        {
            HOTween.To(targetOffset, 3f, "x", 4);
            // currentCameraState = CameraState.ThirdPerson;
    ...
    public void StartTransition2(Transform TransitionTarget)
    {
    ...
    public void StartTransition(Transform TransitionTarget)
    {
        ...
        this.objectsToRotate[0] = null;
        originRotation.x = TransitionTarget.eulerAngles.y;
        inputRotation.x = TransitionTarget.eulerAngles.y;
        mouseXSmooth = inputRotation.x;
        TweenParms tweenparms = new TweenParms().Prop("position", TransitionTarget.rotation * new Vector3(targetOffset.x, 0.0f, -wantedDistance) + TransitionTarget.position + new Vector3(0.0f, targetOffset.y, 0)).Prop("rotation", TransitionTarget.rotation).OnComplete(SwitchTargetThirdPerson, TransitionTarget);

        HOTween.To(this.transform, 0.34f, tweenparms);

    }
    private void SwitchTargetThirdPerson(TweenEvent e)
    {
        Transform TransitionTarget = (Transform)e.parms[0];
        allowRotation = true;
        
      
        //ClampRotation();
        this.objectsToRotate[0] = TransitionTarget;
        currentCameraState = CameraState.ThirdPerson;
       // this.target = (Transform)e.parms[0];
        //this.objectsToRotate[0] = (Transform)e.parms[0];
    }
    #endregion
    #endregion
}
...
public class PopulateDB : MonoBehaviour {
    ...
    public MoveAssetDatabase moveAssetDatabase = null;
    public static PopulateDB instance;
	void Start () {
	...
	public  Pokemon getBasicPokemon(int id){
		...
		foreach(var i in StaticGlobalVariables.moveData.Where (e=>e.PokemonId == id)){
			if(baseMoveCounter < 4){
                ...
                result.moves[baseMoveCounter].Icon = moveAssetDatabase.GetByID(result.moves[baseMoveCounter].Id).Icon;
